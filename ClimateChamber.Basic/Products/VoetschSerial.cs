...
{
    public class VoetschSerial :  DataTunnel<ClimateChamberDataHAL>, IClimateChamberHAL
    {
        public VoetschSerial()
        {
        ...
        {
            ComPort.PortName = validator.ValidationRegex.Match(initString).Result("${com}");
            ComPort.BaudRate = int.Parse(validator.ValidationRegex.Match(initString).Result("${baud}"));
            if (ComPort.IsOpen)
